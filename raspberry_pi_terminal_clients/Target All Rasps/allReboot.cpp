...
int main(void)
{
	...
	cout << "Sending reboot commands..." << endl;
		...
		int line_count = 0;
      	while(getline(newfile, file_content_line)){ //read data from file object and put it into string.
      		...
      		if (ip != my_ip)
      		{
      			...
      			string ssh_command = "sshpass -p '" + password + "' ssh -o StrictHostKeyChecking=no " + username + "@" + ip + " 'sudo reboot' > '" + command_return_file_path +"'";
